...
const server = createServer(async (request, response) => {
  ...
  if (request.method === 'POST' && request.url === '/release') {
    for (const output of pending.keys()) complete(output, 'Late forbidden answer.');
  ...
  try {
    ...
    if (['cancel-queued', 'cancel-running', 'cancel-silent', 'cancel-after'].includes(prompt))
      calls.push({ prompt, modelId: input.model });
    if (prompt === 'cancel-running' || prompt === 'cancel-silent') {
    ...
    response.writeHead(200, { 'content-type': 'text/event-stream' });
    complete(response, prompt === 'cancel-after' ? 'Independent work completes.' : 'OK');
  } catch {
    response.destroy();
  }
});
