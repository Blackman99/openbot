...
## First actual PostgreSQL run and lock-observer correction

Verify33951244638 on remote bcd1f257412b85878f8f665dcd792228cfefcf44 (exact tree6b952ea matches acceptedb71fa89) completed2026-09-05T07:01:18Z with seven jobs successful and only postgres-bots101266597945 failing. Identity8, ACL9 and avatar8 passed; versions10/11 passed. The concurrent edit/restore case failed in waitForBlocked at line151 before release/outcome assertions: it assumed both row-lock waiters directly named the original transaction as blocker. PostgreSQL queues the second tuple waiter behind the first waiter. Existing actual-green identity/ACL/avatar tests already observe that chain.

Test-only candidate523dd2e stages the first edit's arrival under the still-held original lock, records its PID, then starts restore and observes it blocked on the edit PID before releasing the original lock. Both operations still overlap; exact one-success/one-version-conflict, immutable-original, two-version and one-audit assertions remain unchanged. The early rejection observer does not replace either original promise. No production behavior, timeout or retry changes. Independent Standards and test-intent/Spec review is CLEAN. Formatting/API types passed and eleven native cases explicitly skip locally. Real native retry remains required; no BOT-03-E1 closure is claimed.

Dedicated correction integration: merge8ad6267fed005173ad9fc3c376dd66b164f69b24, tree90cdef416d89c7a218df6642f1b7f13df3daf3f8. Complete pnpm verify passed867 code tests (API88+302,Web52+425),26 ordinary browsers and one signed OIDC, formatting, zero-error/zero-warning types and both builds. No additional source changes; actual11 native retry remains open.
