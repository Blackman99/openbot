<script lang="ts">
  ...
  let { canCancel, canConfirm, idempotencyKey, expectedRunId, actionUrl, action }: {
    canCancel: boolean; canConfirm: boolean; idempotencyKey: string; expectedRunId: string;
  ...
  const cancelTask:SubmitFunction=({formData,cancel})=>{
    ...
    return async({result,update})=>{
      const preserve=()=>{unconfirmed=captured;transportError='The cancellation could not be confirmed. Confirm the unchanged command to check its saved result.';};
...
{#if canCancel || confirm}
  <section aria-labelledby="cancel-heading">
    <h2 id="cancel-heading">Cancel task</h2>
    <p>Stops this task and any unfinished work it started. Saved output stays available and is marked interrupted.</p>
    <form method="POST" action={actionUrl} use:enhance={cancelTask}>
      ...
      {#if uncertain}<p>The original cancellation command is preserved until its outcome is confirmed.</p>{/if}
      <button disabled={submitting||action?.conflict}>{submitting?'Confirming…':uncertain?'Confirm unchanged cancellation':'Cancel task'}</button>
    </form>
  </section>
{/if}
<style>
  section { margin-top: 1.5rem; } h2 { font-size: 1.25rem; }
  button { padding:.75rem; border:1px solid #526171; border-radius:.4rem; font:inherit; background:#2d1920; color:#ffb4b4; cursor:pointer; }
