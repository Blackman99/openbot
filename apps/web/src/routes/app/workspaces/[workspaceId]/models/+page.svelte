...
<svelte:head><title>Workspace models · {data.workspace.name} · OpenBot</title></svelte:head>
...
{/snippet}
<main>
  <a href={`/app/workspaces/${encodeURIComponent(data.workspace.id)}`}>← {data.workspace.name}</a>
  <h1>Workspace models</h1>
  <p>Shared connections are available to workspace members. Owners and administrators manage connection settings.</p>
  ...
  {#if data.canManage}
    <section aria-labelledby="new-model">
      <h2 id="new-model">Add a shared model</h2>
      <p>Saving tests live text streaming and structured actions. A model with working text can be saved even if structured actions are unavailable.</p>
      <form method="POST" action="?/save">{@render fields(undefined)}</form>
    </section>
  {/if}
  {#if data.connections.length === 0}<p>No shared models yet.</p>{/if}
  {#each data.connections as connection (connection.id)}
    <article aria-labelledby={`model-${connection.id}`}>
      ...
      {#if data.canManage && connection.settings}
        <p>API key: {connection.settings.apiKeyConfigured ? 'configured' : 'not configured'}</p>
        <p>Configured headers: {connection.settings.headerNames.join(', ') || 'none'}</p>
        <details><summary>Test evidence</summary><pre>{JSON.stringify(connection.settings.lastProbe, null, 2)}</pre></details>
        <details><summary>Edit connection</summary><form method="POST" action="?/save">{@render fields(connection.settings)}</form></details>
      {/if}
      <div class="controls">
        <form method="POST" action="?/test"><input type="hidden" name="id" value={connection.id} /><button disabled={connection.availability !== 'available'}>Test again</button></form>
        {#if data.canManage}
          <form method="POST" action="?/disable"><input type="hidden" name="id" value={connection.id} /><button disabled={connection.availability !== 'available'}>Disable</button></form>
        {/if}
      </div>
    </article>
  {/each}
