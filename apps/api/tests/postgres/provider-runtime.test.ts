...
postgresDescribe('personal connections using the deployed restricted database role', () => {
  ...
  it('persists encrypted owner-only connections, rejects stale revisions and rolls back when audit insertion fails', async () => {
    ...
    const created = await service.save(owner, {
      ...
      headers: { 'x-api-key': 'ci-header-secret' },
    });
