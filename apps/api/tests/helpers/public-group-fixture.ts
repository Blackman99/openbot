...
export async function publicGroupFixture(cleanup: Array<() => Promise<unknown>>) {
  ...
  async function bearer(scopes: ApiTokenScope[], actor = base.owner.user.id) {
    const created = await tokens.create(actor, base.owner.workspace.id, {
      name: 'Public group client',
      scopes,
      expiresAt: new Date(Date.now() + 86_400_000).toISOString(),
    });
    return { authorization: `Bearer ${created.secret}` };
  }
  return { ...base, publicApp, sessionApp, tokens, groups, groupBots, groupRouting, bearer };
}
