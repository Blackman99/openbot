...
function resolvedConcurrentRuns(
  maxConcurrentRuns: number | null | undefined,
  executionPolicy: unknown,
): number {
  if (typeof maxConcurrentRuns === 'number') return maxConcurrentRuns;
  return parseExecutionPolicy(executionPolicy).maxConcurrentRuns ?? DEFAULT_GROUP_CONCURRENT_RUNS;
}

export class PostgresGroupRepository implements GroupRepository {
  constructor(private readonly pool: SqlPool) {}
  async update(record: GroupMetadataWrite, admission?: TransactionAdmission): Promise<Group> {
    return this.manage(
      record,
      async (connection, group) => {
        const current = (
          await connection.query<{ max_concurrent_runs: number | null }>(
            'SELECT max_concurrent_runs FROM groups WHERE id=$1',
            [record.groupId],
          )
        ).rows[0];
        const changedFields = (
          ['name', 'description', 'visibility', 'maxConcurrentRuns'] as const
        ).filter((field) => {
          if (record.changes[field] === undefined) return false;
          if (field === 'maxConcurrentRuns')
            return record.changes.maxConcurrentRuns !== current?.max_concurrent_runs;
          return record.changes[field] !== group[field];
        });
        if (!changedFields.length) return group;
        const updated = { ...group, ...record.changes, updatedAt: record.occurredAt };
        await connection.query(
          'UPDATE groups SET name=$3,description=$4,visibility=$5,updated_at=$6,max_concurrent_runs=$7 WHERE workspace_id=$1 AND id=$2',
          [
            record.workspaceId,
            record.groupId,
            updated.name,
            ...
            updated.visibility,
            record.occurredAt,
            record.changes.maxConcurrentRuns === undefined
              ? (current?.max_concurrent_runs ?? null)
              : record.changes.maxConcurrentRuns,
          ],
        );
        await connection.query(
  ...
  async changeRole(
    record: GroupMemberWrite,
    admission?: TransactionAdmission,
  ): Promise<GroupMember> {
    return this.mutateMember(record, record.role, admission);
  }
  async removeMember(record: GroupMemberRemoval, admission?: TransactionAdmission): Promise<void> {
    await this.mutateMember(record, null, admission);
  }
  ...
  private async manage<T>(
    ...
    admission?: TransactionAdmission,
    permission: 'manage' | 'archive' = 'manage',
  ): Promise<T> {
    const connection = await this.pool.connect();
    try {
      await connection.query('BEGIN');
      const group = await lockAuthorizedGroup(connection, record, permission);
  ...
  list(
    ...
    return this.metadata(actorId, workspaceId, undefined, options, admission);
  }
  async get(
    actorId: string,
    workspaceId: string,
    ...
    const group = (
      await this.metadata(actorId, workspaceId, groupId, { includeArchived: true }, admission)
    )[0];
  ...
  async inspect(
    ...
    try {
      ...
      const row = (
        ...
        >(
          `SELECT g.id,g.workspace_id,g.name,g.description,g.visibility,g.created_at,g.updated_at,g.archived_at,g.max_concurrent_runs,g.execution_policy,gm.role FROM groups g INNER JOIN workspace_memberships wm ON wm.workspace_id=g.workspace_id AND wm.user_id=$2 LEFT JOIN group_memberships gm ON gm.group_id=g.id AND gm.user_id=$2 WHERE g.workspace_id=$1 AND g.id=$3 AND (g.visibility='workspace' OR gm.user_id IS NOT NULL)`,
          [workspaceId, actorId, groupId],
        )
      ).rows[0];
      if (!row) throw new GroupAccessError();
      await admission?.(connection);
      await connection.query('COMMIT');
      return {
        ...groupDto(row),
        ...
        policy: {
          maxConcurrentRuns: resolvedConcurrentRuns(row.max_concurrent_runs, row.execution_policy),
        },
      };
    } catch (error) {
      await connection.query('ROLLBACK');
  ...
  async archive(record: GroupArchiveWrite, admission?: TransactionAdmission): Promise<PublicGroup> {
    return this.manage(
      record,
      async (connection) => {
        const current = (
          await connection.query<{
            ...
            `SELECT g.name,g.description,g.visibility,g.created_at,g.updated_at,g.archived_at,g.max_concurrent_runs,g.execution_policy,gm.role FROM groups g INNER JOIN group_memberships gm ON gm.group_id=g.id AND gm.user_id=$3 WHERE g.workspace_id=$1 AND g.id=$2`,
            [record.workspaceId, record.groupId, record.actorId],
          )
        ).rows[0];
        if (!current) throw new GroupAccessError();
        const archivedAt = current.archived_at ?? record.occurredAt;
        if (!current.archived_at) {
          await connection.query(
            'UPDATE groups SET archived_at=$3,updated_at=$3 WHERE workspace_id=$1 AND id=$2',
            [record.workspaceId, record.groupId, record.occurredAt],
          );
          await connection.query(
            "INSERT INTO audit_events (id,event_type,actor_user_id,occurred_at,metadata) VALUES ($1,'group.archived',$2,$3,$4::jsonb)",
            [
              record.auditId,
              record.actorId,
              record.occurredAt,
              JSON.stringify({ groupId: record.groupId, workspaceId: record.workspaceId }),
            ],
          );
        }
        return {
          ...
          updatedAt: current.archived_at ?? record.occurredAt,
          archivedAt,
          policy: {
            maxConcurrentRuns: resolvedConcurrentRuns(
  ...
  async create(record: GroupCreate, admission?: TransactionAdmission): Promise<Group> {
    const connection = await this.pool.connect();
    try {
      await connection.query('BEGIN');
