...
import {
  ...
  type TokenBudgetWarningCrossing,
  type TokenReservationDecision,
  type TokenBudgetLayer,
} from './token-budget.js';

function warningBody(warning: TokenBudgetWarningCrossing) {
  const label =
    warning.dimension === 'inputTokens'
      ? 'Input-token'
      : warning.dimension === 'outputTokens'
        ? 'Output-token'
        : 'Token';
  const unit = warning.dimension === 'totalTokens' ? ' total' : '';
  return warning.hard
...
export async function appendTokenBudgetWarnings(
  ...
  for (const warning of tokenBudgetWarningCrossings(warnings))
    await appendTokenBudgetWarning(connection, input, warning);
}

async function appendTokenBudgetWarning(
  connection: SqlConnection,
  input: {
    taskId: string;
    workspaceId: string;
  ...
  warning: TokenBudgetWarningCrossing,
) {
  const idempotencyKey = `task-token-warning:${input.taskId}:${warning.source}:${warning.dimension}`;
  const existing = (
