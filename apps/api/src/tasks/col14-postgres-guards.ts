// Repeatable overlay after 0040. It does not consume a ledger number and does
// not rewrite published 0017/0022/0023/0033/0034/0035/0036/0037 statement lists.
// Applied last so COL-08/COL-11/COL-12 do not overwrite waiting_child or
// child_result recognition. Automatic continuation origins stay the three
// COL-10/COL-11 values.

import { COL14_DELEGATION_REQUIRES_VERSION } from './delegate-schema.js';

export { COL14_DELEGATION_REQUIRES_VERSION };

export const COL14_DELEGATION_POSTGRES_GUARDS = [
  `CREATE OR REPLACE FUNCTION task_has_child_result_receipt(target uuid, run_id uuid, actor uuid)
RETURNS boolean
LANGUAGE sql
VOLATILE
SECURITY DEFINER
      ...
      ELSE
        IF ROW(NEW.id,NEW.task_id,NEW.attempt,NEW.created_at)
          ...
          OR NOT ((OLD.status='queued' AND NEW.status IN ('running','failed','cancelled','paused'))
            ...
            OR (OLD.status='waiting_child' AND NEW.status='cancelled')) THEN
      ...
      IF parent.id IS NULL OR latest.id IS NULL
        ...
        OR (latest.attempt>1 AND NOT EXISTS (SELECT 1 FROM task_retry_commands c
          ...
          AND COALESCE(task_queued_audit_metadata(latest.id)->>'origin','')
            NOT IN ('manual_resume','budget_grant','provider_retry','model_fallback','worker_recovery','child_result')) THEN
