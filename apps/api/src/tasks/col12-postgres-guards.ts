...
export const COL12_LIMIT_HELPERS = [
...
AS $$
  SELECT EXISTS (
    ...
    JOIN task_limit_grants g ON g.task_id=target AND g.actor_user_id=actor
    WHERE a.event_type='task.queued'
      ...
      AND previous.status IN ('waiting_budget','failed','paused')
...
export const COL12_LIMITS_POSTGRES_GUARDS = [
  ...
  `CREATE OR REPLACE FUNCTION protect_task() RETURNS TRIGGER LANGUAGE plpgsql AS $$
    ...
    BEGIN
      IF TG_OP='UPDATE' THEN
        IF NEW.status<>'cancelled' AND NEW.status<>'paused' AND NEW.status<>'waiting_budget'
          AND NOT lock_task_ancestry(NEW.id, (OLD.status='paused' OR OLD.status='waiting_budget') AND NEW.status='queued') THEN
        ...
        IF (to_jsonb(NEW)-'status') IS DISTINCT FROM (to_jsonb(OLD)-'status')
          OR NOT ((OLD.status='queued' AND NEW.status IN ('running','failed','cancelled','paused','waiting_budget'))
            OR (OLD.status='running' AND NEW.status IN ('completed','failed','cancelled','paused','waiting_budget'))
        ...
        END IF;
        IF NEW.status<>'waiting_budget' AND (latest.id IS NULL OR latest.status<>NEW.status) THEN
          RAISE EXCEPTION USING ERRCODE='23514', MESSAGE='Task state must match its current Run';
        END IF;
        IF NEW.status='waiting_budget' AND (latest.id IS NULL OR latest.status NOT IN ('queued','running','failed','paused','waiting_budget')) THEN
          RAISE EXCEPTION USING ERRCODE='23514', MESSAGE='waiting_budget Task requires a retained current Run';
        END IF;
  ...
  `CREATE OR REPLACE FUNCTION protect_task_run() RETURNS TRIGGER LANGUAGE plpgsql AS $$
    ...
    BEGIN
      IF NEW.status<>'cancelled' AND NEW.status<>'paused' AND NEW.status<>'waiting_budget'
        AND NOT lock_task_ancestry(NEW.task_id, TG_OP='INSERT') THEN
        ...
        IF parent.id IS NULL OR NEW.status<>'queued' OR NEW.created_at<parent.created_at
          OR NOT ((NEW.attempt=1 AND latest.id IS NULL AND parent.status='queued')
            ...
            OR (latest.id IS NOT NULL AND parent.status='paused' AND latest.status='paused'
              AND NEW.attempt::bigint=latest.attempt::bigint+1 AND NEW.created_at>=latest.finished_at)
            OR (latest.id IS NOT NULL AND parent.status='waiting_budget'
              AND latest.status IN ('waiting_budget','failed','paused')
              AND NEW.attempt::bigint=latest.attempt::bigint+1
              AND NEW.created_at>=COALESCE(latest.finished_at,NEW.created_at))) THEN
      ...
      ELSE
        IF ROW(NEW.id,NEW.task_id,NEW.attempt,NEW.created_at)
          ...
          OR NOT ((OLD.status='queued' AND NEW.status IN ('running','failed','cancelled','paused','waiting_budget'))
            OR (OLD.status='running' AND NEW.status IN ('completed','failed','cancelled','paused','waiting_budget'))) THEN
          RAISE EXCEPTION USING ERRCODE='55000', MESSAGE='Run identity and terminal state are immutable';
        END IF;
        IF parent.id IS NULL OR (parent.status<>OLD.status AND NEW.status<>'waiting_budget') OR latest.id<>OLD.id THEN
  ...
  `CREATE OR REPLACE FUNCTION require_current_task_run() RETURNS TRIGGER LANGUAGE plpgsql AS $$
    ...
    BEGIN
      ...
      IF parent.id IS NULL OR latest.id IS NULL
        ...
        OR (parent.status='waiting_budget' AND latest.status NOT IN ('queued','running','failed','paused','waiting_budget'))
