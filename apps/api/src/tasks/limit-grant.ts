...
import { LIMIT_DIMENSIONS, type LimitDimension } from './limit-view.js';
import { requireWorkspaceManager } from './limit-policy.js';
...
export function limitGrantCommand(input: unknown): LimitGrantCommand {
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw new LimitInputError();
  ...
  if (
    ...
    !LIMIT_DIMENSIONS.includes(value.dimension as LimitDimension) ||
    typeof value.limit !== 'number' ||
    !Number.isInteger(value.limit)
  )
    throw new LimitInputError();
  const dimension = value.dimension as LimitDimension;
  const max =
    dimension === 'durationMs'
      ...
      : dimension === 'turns'
        ...
        : dimension === 'depth'
          ? 8
          : 32;
  const min = dimension === 'durationMs' ? 1 : 0;
  if (value.limit < min || value.limit > max) throw new LimitInputError();
...
export async function grantTaskLimit(
  ...
  if (!selected) throw new LimitAccessError();
  ...
  const prior = (
    await connection.query<{
      ...
      'SELECT id,dimension,previous_limit,granted_limit FROM task_limit_grants WHERE task_id=$1 AND actor_user_id=$2 AND idempotency_key=$3',
    ...
    const successor = (
      await connection.query<{ run_id: string; attempt: number }>(
        `SELECT r.id AS run_id, r.attempt
         ...
         ORDER BY a.occurred_at, r.attempt
         LIMIT 1`,
        [taskId, access.actorUserId],
      )
    ).rows[0];
    return {
      grantId: prior.id,
      ...
      runId: successor?.run_id ?? null,
      attempt: successor ? Number(successor.attempt) : null,
    };
  }
  const snapshot = await loadSnapshot(connection, taskId);
  if (!snapshot) throw new LimitConflictError('task_limit_snapshot_missing');
  const effective = await loadEffectiveLimits(connection, taskId, snapshot);
  const previous =
    command.dimension === 'durationMs'
      ? effective.durationMs
      : command.dimension === 'turns'
        ? effective.turns
        : command.dimension === 'depth'
          ? effective.depth
          : effective.handoffs;
  if (command.limit <= previous) throw new LimitConflictError('task_limit_grant_not_increased');
  const occurredAt = now();
  const grantId = randomUUID();
  await connection.query(
    `INSERT INTO task_limit_grants(id,task_id,actor_user_id,idempotency_key,dimension,previous_limit,granted_limit,created_at)
  ...
  if (selected.status === 'waiting_budget') {
    ...
    if (!latest) throw new LimitConflictError('task_limit_state_conflict');
    if (!(await lockTaskAncestry(connection, taskId, { allowPausedTarget: true })))
      throw new LimitConflictError('task_limit_paused_ancestor');
    ...
    if (!written.scheduled)
      throw new LimitConflictError(
        written.reason === 'cancelled' ? 'task_limit_paused_ancestor' : 'task_limit_state_conflict',
      );
    runId = written.runId;
    attempt = (
