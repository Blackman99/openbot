// Repeatable overlay after 0047. It does not consume a ledger number and does
// not rewrite published 0017–0049 statement lists. Applied last after COL-16
// so human-decision receipts survive. Automatic continuation origins stay
// provider_retry, model_fallback, and worker_recovery.

import { COL19_HUMAN_REQUEST_REQUIRES_VERSION } from './human-request-schema.js';

export { COL19_HUMAN_REQUEST_REQUIRES_VERSION };

export const COL19_HUMAN_REQUEST_POSTGRES_GUARDS = [
  `CREATE OR REPLACE FUNCTION task_has_human_decision_receipt(target uuid, run_id uuid, actor uuid)
RETURNS boolean
LANGUAGE sql
VOLATILE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    JOIN task_human_requests req ON req.task_id=target AND req.source_run_id=previous.id
    JOIN task_human_decisions d ON d.request_id=req.id
    WHERE a.event_type='task.queued'
      AND a.actor_user_id=actor
      AND a.metadata->>'taskId'=target::text
      AND a.metadata->>'runId'=run_id::text
      AND a.metadata->>'origin'='human_decision'
      AND previous.status IN ('waiting_input','waiting_approval')
      AND previous.attempt::bigint+1=next_run.attempt
      AND req.resolved_at IS NOT NULL
  )
$$`,
  'REVOKE ALL ON FUNCTION task_has_human_decision_receipt(uuid,uuid,uuid) FROM PUBLIC',
        ...
        IF parent.id IS NULL OR NEW.status<>'queued' OR NEW.created_at<parent.created_at
          OR NOT ((NEW.attempt=1 AND latest.id IS NULL AND parent.status='queued')
            ...
            OR (latest.id IS NOT NULL AND parent.status='waiting_budget' AND latest.status IN ('failed','paused')
              AND NEW.attempt::bigint=latest.attempt::bigint+1 AND NEW.created_at>=latest.finished_at)
            OR (latest.id IS NOT NULL AND parent.status='waiting_child' AND latest.status='waiting_child'
              AND NEW.attempt::bigint=latest.attempt::bigint+1 AND NEW.created_at>=latest.finished_at)
            OR (latest.id IS NOT NULL AND parent.status IN ('waiting_input','waiting_approval') AND latest.status=parent.status
      ...
      ELSE
        IF ROW(NEW.id,NEW.task_id,NEW.attempt,NEW.created_at)
          ...
          OR NOT ((OLD.status='queued' AND NEW.status IN ('running','failed','cancelled','paused'))
            OR (OLD.status='running' AND NEW.status IN ('completed','failed','cancelled','paused','waiting_child','waiting_input','waiting_approval'))
            OR (OLD.status='waiting_child' AND NEW.status='cancelled')
            OR (OLD.status IN ('waiting_input','waiting_approval') AND NEW.status='cancelled')) THEN
        ...
        END IF;
        IF NEW.status='waiting_child' AND (
          ROW(NEW.started_at,NEW.claim_token,NEW.deadline_at,NEW.provider_scope_kind,
            NEW.provider_scope_id,NEW.connection_id,NEW.connection_revision,NEW.protocol,NEW.model_id)
            IS DISTINCT FROM ROW(OLD.started_at,OLD.claim_token,OLD.deadline_at,OLD.provider_scope_kind,
              OLD.provider_scope_id,OLD.connection_id,OLD.connection_revision,OLD.protocol,OLD.model_id)
          ...
          OR NOT EXISTS (SELECT 1 FROM task_delegations d
            WHERE d.parent_run_id=NEW.id AND d.parent_task_id=NEW.task_id)) THEN
          RAISE EXCEPTION USING ERRCODE='23514', MESSAGE='waiting child Run requires its delegation receipt, retained claim and usage';
        END IF;
        IF NEW.status IN ('waiting_input','waiting_approval') AND (
          ROW(NEW.started_at,NEW.claim_token,NEW.deadline_at,NEW.provider_scope_kind,
            NEW.provider_scope_id,NEW.connection_id,NEW.connection_revision,NEW.protocol,NEW.model_id)
            IS DISTINCT FROM ROW(OLD.started_at,OLD.claim_token,OLD.deadline_at,OLD.provider_scope_kind,
              OLD.provider_scope_id,OLD.connection_id,OLD.connection_revision,OLD.protocol,OLD.model_id)
          ...
          ROW(NEW.input_tokens,NEW.output_tokens,NEW.error_code,NEW.output_event_id) IS DISTINCT FROM
            ROW(OLD.input_tokens,OLD.output_tokens,OLD.error_code,OLD.output_event_id)
          OR NEW.finished_at IS NULL
          OR NOT EXISTS (SELECT 1 FROM task_human_requests r
            WHERE r.source_run_id=NEW.id AND r.task_id=NEW.task_id
              AND r.kind=CASE NEW.status WHEN 'waiting_input' THEN 'input' ELSE 'approval' END)) THEN
          RAISE EXCEPTION USING ERRCODE='23514', MESSAGE='waiting human Run requires its request receipt, retained claim and usage';
        END IF;
  ...
  `CREATE OR REPLACE FUNCTION protect_task() RETURNS TRIGGER LANGUAGE plpgsql AS $$
    ...
    BEGIN
      IF TG_OP='UPDATE' THEN
        IF NEW.status<>'cancelled' AND NEW.status<>'paused' AND NEW.status<>'waiting_budget' AND NEW.status<>'waiting_child' AND NEW.status<>'waiting_input' AND NEW.status<>'waiting_approval' AND NOT lock_task_ancestry(NEW.id, (OLD.status='paused' OR OLD.status='waiting_budget' OR OLD.status='waiting_child' OR OLD.status='waiting_input' OR OLD.status='waiting_approval') AND NEW.status='queued') THEN
        ...
        IF NOT identity_stable
          ...
          OR NOT ((OLD.status='queued' AND NEW.status IN ('running','failed','cancelled','paused','waiting_budget'))
            ...
            OR (OLD.status='waiting_child' AND NEW.status IN ('queued','cancelled'))
            OR (OLD.status IN ('waiting_input','waiting_approval') AND NEW.status IN ('queued','cancelled'))) THEN
        ...
        END IF;
        IF OLD.status IN ('waiting_input','waiting_approval') AND NEW.status='queued' AND NOT (
          latest.status='queued'
          AND (
            task_has_human_decision_receipt(NEW.id, latest.id, NEW.execution_user_id)
            OR (
              COALESCE(task_queued_audit_metadata(latest.id)->>'origin','')='human_decision'
              AND EXISTS (
                SELECT 1 FROM task_runs previous
                WHERE previous.task_id=NEW.id
                  AND previous.id::text=task_queued_audit_metadata(latest.id)->>'sourceRunId'
          ...
          RAISE EXCEPTION USING ERRCODE='55000', MESSAGE='Task human decision requires a new Run and its immutable receipt';
        END IF;
    ...
    BEGIN
      ...
      IF parent.id IS NULL OR latest.id IS NULL
        ...
        OR (latest.attempt>1 AND NOT EXISTS (SELECT 1 FROM task_retry_commands c
          ...
          AND NOT task_has_human_decision_receipt(target, latest.id, parent.execution_user_id)
          AND COALESCE(task_queued_audit_metadata(latest.id)->>'origin','')
            NOT IN ('manual_resume','budget_grant','provider_retry','model_fallback','worker_recovery','child_result','handoff','human_decision')) THEN
      ...
      IF NEW.ledger_event_id IS NOT NULL AND NOT EXISTS (
        SELECT 1 FROM conversation_events e WHERE e.id=NEW.ledger_event_id AND e.conversation_id=target AND e.sequence=NEW.sequence
          AND ((NEW.event_type='message.changed' AND e.message_id IS NOT NULL)
            OR (NEW.event_type='conversation.invalidated' AND e.event_type IN ('bot.joined','bot.removed','task.limit.warning','task.handoff','task.input.requested','task.approval.requested','task.human.decided')))) THEN
