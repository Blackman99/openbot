...
The original execution human or a current group owner/administrator can cancel a queued or running Task from its details. The command stops every unfinished descendant and retains committed output as interrupted. Stopping does not require renewed model-use admission. See [Task cancellation](task-cancellation.md) for the stable command, private output reader and required worker-drain upgrade procedure.
...
Restarting the worker processes durable queued Runs. This version leaves an uncertain already-running Run unchanged after an unexpected process exit; automatic retries, fallback and crash recovery are still separate implementation tickets. Manual retry applies to failed Tasks and does not reclaim an uncertain running attempt or restart cancelled work.
